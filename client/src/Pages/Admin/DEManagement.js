...
export default function DEManagement() {
	...
	const fetchData = async () => {
		await request('executives', {
			method: 'GET',
			before: () => setloading(true),
			success: (d) => {
				setData(d.executives);
			},
		...
		<PageWrapper>
			<div className='col-8 customer-management'>
				<div className='breadscrumb'>
					<Link to='/'>Home</Link> &nbsp; &gt; &nbsp;
					<Link>Delivery Executive Management</Link>
				</div>
				<div className='my-3'>
					<h5 className='head text-themed f600'>Delivery Executive Management</h5>
				</div>
				<ConfirmationModal
					isOpen={isModalOpen}
